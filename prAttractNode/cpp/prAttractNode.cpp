...
MObject prAttractNode::aInputShape;
...
MStatus prAttractNode::initialize()
{
    ...
    // aInputShape
    aInputShape = gAttr.create( "inputShape", "inputShape" );
    ...
    addAttribute( aInputShape );
	attributeAffects( prAttractNode::aInputShape, prAttractNode::outputGeom );
    // aInputMatrix
...
MStatus prAttractNode::deform( MDataBlock& block, MItGeometry& iter, const MMatrix& mat, unsigned int multiIndex )
{
	...
	// aInputShape
	MDataHandle hInputShape = block.inputValue( aInputShape );
	MPlug plugInputShape( thisNode, aInputShape );
	if( plugInputShape.isConnected() == false )
	{
		if( warnings )
			MGlobal::executeCommand( "print \"\\nMissing incoming connection to aInputShape.\"" );
		return stat;
	}
	...
	plugInputShape.connectedTo( plugarInputShape, true, false, &stat);
	if ( stat != MS::kSuccess ) return stat;
	...
	MFnData::Type typeInputShape = hInputShape.type();
	...
	if( typeInputShape == MFnMeshData::kMesh )
	{
		strInputShapeType = "mesh";
		MObject oInputPoly;
		oInputPoly = hInputShape.data();
		stat = fnTargetPoly.create( oInputPoly, matInputMatrix );
		if(stat!=MS::kSuccess)return stat;
		stat = fnMesh.setObject( oInputPoly );
		if(stat!=MS::kSuccess)return stat;
	}
	else if( typeInputShape == MFnNurbsCurveData::kNurbsCurve )
	{
		strInputShapeType = "curve";
		stat = fnTargetNurbsCurve.setObject( dagPathInputShape );
		if(stat!=MS::kSuccess)return stat;
	}
	else if( typeInputShape == MFnNurbsSurfaceData::kNurbsSurface )
	{
		strInputShapeType = "nurbs";
		stat = fnTargetNurbsSurface.setObject( dagPathInputShape );
		if(stat!=MS::kSuccess)return stat;
	}
	...
	// loop
	#ifdef _OPENMP
	#pragma omp parallel for if(nPoints>900 && strInputShapeType=="mesh") \
	...
	for( int i=0; i<nPoints; i++ )
	{
		...
		else if( strInputShapeType == "curve" )
		{
...
void prAttractNodeAETemplate()
{
	...
	ae+= "editorTemplate -suppress \"inputShape\";\n";
